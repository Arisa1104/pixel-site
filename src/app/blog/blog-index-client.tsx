...
export default function BlogIndexClient({ posts }: { posts: Post[] }) {
  ...
  const filteredPosts = selectedCategory === "All"
    ? posts
    : posts.filter((p) => p.frontmatter.category?.toLowerCase() === selectedCategory.toLowerCase());
      ...
      <main className="mx-auto w-full max-w-6xl px-6 py-12">
        ...
        <div className="mx-auto mt-12 flex max-w-3xl flex-wrap justify-center gap-3 border-b border-ink/10 pb-5">
          {categories.map((c) => (
            <button
              ...
              className={`rounded-full px-4 py-2 font-sans text-xs tracking-widest uppercase transition-colors ${
                c === selectedCategory
                  ? "bg-accent-rose text-paper"
        ...
        <section className="mt-14 grid gap-14 lg:grid-cols-12 lg:items-start">
          ...
          {featured ? (
            ...
            <div className="lg:col-span-8 text-center text-ink/50">
